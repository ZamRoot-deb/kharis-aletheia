/* KHARIS & ALETHEIA — site configuration (shared across every page)
   Ported verbatim from ../config.js. Anything the owner must actually supply before
   launch is listed in KA_CONFIG.placeholders. The five owner-facing values below read
   NEXT_PUBLIC_* env overrides, falling back to the same placeholder values the old
   config.js shipped with. */
import type { KaConfig } from './types';

export const KA_CONFIG: KaConfig = {
  brand: 'KHARIS & ALETHEIA',
  currency: 'GBP',
  currencySymbol: '£',

  /* PLACEHOLDER — owner's real inbox. mailto: fallback + order notices go here. */
  contactEmail: process.env.NEXT_PUBLIC_CONTACT_EMAIL || 'hello@kharisandaletheia.com',

  /* POST JSON endpoint for kaForms.submit (Formspree-style). '' => mailto: fallback.
     Must be HTTPS — kaForms.submit refuses any non-https:// endpoint and falls back
     to mailto: rather than send form PII in cleartext. */
  ...
  payment: {
    provider: process.env.NEXT_PUBLIC_PAYMENT_PROVIDER || 'request',
  ...
  siteUrl: process.env.NEXT_PUBLIC_SITE_URL || 'https://www.kharisandaletheia.com',
