...
export async function openPage(b, { width = 1440, height = 900, mobile = false, theme = 'dark' } = {}) {
  ...
  const page = {
    ...
    async clickExpr(expr) {
      // expr is a JS expression evaluating to an element; real-mouse click its centre
      const r = await page.eval(`(()=>{const e=(${expr});if(!e)return null;e.scrollIntoView({block:'center',inline:'center',behavior:'instant'});const b=e.getBoundingClientRect();if(b.width<2||b.height<2)return {miss:1};return {x:b.left+b.width/2,y:b.top+b.height/2}})()`)
      if (!r) return 'MISSING'
      if (r.miss) return 'ZEROSIZE'
      await sleep(60)
      const r2 = await page.eval(`(()=>{const e=(${expr});if(!e)return null;const b=e.getBoundingClientRect();return {x:b.left+b.width/2,y:b.top+b.height/2}})()`)
      const pt = r2 || r
