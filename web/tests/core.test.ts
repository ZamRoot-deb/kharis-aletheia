/* KHARIS & ALETHEIA — KA_CORE tests (per CONTRACT-NEXT.md)
   Ported from ../../tests/core.test.js. core.ts now imports KA_CONFIG/KA_PRODUCTS
   directly (no more window/vm-sandbox stub needed — that was only required because
   the old config.js/products.js were browser globals). _init() is still exercised
   here for the "hostile promo" test, exactly as the old suite did.
   Run: node --test tests/*.test.ts */

import test from 'node:test';
import assert from 'node:assert';

import { KA_CORE } from '../lib/core.ts';
import { KA_CONFIG as REAL_CONFIG } from '../lib/config.ts';
import { KA_PRODUCTS as REAL_PRODUCTS } from '../lib/products.ts';

assert.ok(REAL_CONFIG, 'config.ts must export KA_CONFIG');
assert.ok(Array.isArray(REAL_PRODUCTS) && REAL_PRODUCTS.length > 0, 'products.ts must export a non-empty KA_PRODUCTS');
...
test('shippingOptions() free-over threshold: 74.99 is not free, 75 is free (UK zone)', () => {
  ...
  assert.ok(below.every((o: any) => o.price > 0), 'no method should be free just under the threshold');
  const at = KA_CORE.shippingOptions('GB', 75);
  assert.ok(at.some((o: any) => o.price === 0), 'the cheapest method should be free at the threshold');
  // only the cheapest method drops to 0 — the rest keep their price
  const cheapestId = at.slice().sort((a: any, b: any) => a.price - b.price)[0].id;
  for (const o of at) {
...
test('totals() total and discount are never negative, even if a promo would exceed the subtotal', () => {
  const savedConfig = (KA_CORE as any)._config || REAL_CONFIG;
  ...
  try {
    ...
    KA_CORE._init({ config: REAL_CONFIG, products: REAL_PRODUCTS }); // restore for later tests
  }
});
