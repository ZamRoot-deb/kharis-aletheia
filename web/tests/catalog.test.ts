/* KHARIS & ALETHEIA — catalog data-integrity tests (per CONTRACT-NEXT.md)
   Ported from ../../tests/catalog.test.js. products.ts now exports its data as real
   ESM bindings (no more window/vm-sandbox stub needed). Image existence is checked
   against web/public/<img> — the Next port's actual asset location — instead of the
   old static site's root.
...
import { KA_PRODUCTS, KA_CAT_LABEL, KA_COLLECTIONS, KA_MAKERS, KA_LOOKS } from '../lib/products.ts';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const PUBLIC_ROOT = path.join(__dirname, '..', 'public');

test('products.ts exports every documented value', () => {
...
test('every product img file exists on disk under public/', () => {
  for (const p of KA_PRODUCTS) {
    const full = path.join(PUBLIC_ROOT, p.img);
    assert.ok(fs.existsSync(full) && fs.statSync(full).isFile(), `${p.id}: image not found at public/${p.img}`);
  }
});
...
test('every collection has {slug,name,tag,blurb,img} and its image exists on disk', () => {
  ...
  for (const c of KA_COLLECTIONS) {
    for (const field of ['slug', 'name', 'tag', 'blurb', 'img'] as const) {
    ...
    const full = path.join(PUBLIC_ROOT, c.img);
    assert.ok(fs.existsSync(full) && fs.statSync(full).isFile(), `collection "${c.slug}": image not found at public/${c.img}`);
  }
});

test('every required collection slug from CONTRACT-NEXT.md is present', () => {
  const slugs = new Set(KA_COLLECTIONS.map((c) => c.slug));
...
test('every maker has the documented fields, a valid status, and a real collection slug', () => {
  ...
  for (const m of KA_MAKERS) {
    for (const field of ['slug', 'name', 'series', 'city', 'bio', 'long'] as const) {
...
test('every required maker slug from CONTRACT-NEXT.md is present', () => {
  const slugs = new Set(KA_MAKERS.map((m) => m.slug));
...
test('every look has {id,img,title,caption,productIds} with an existing image and non-empty productIds', () => {
  ...
  for (const l of KA_LOOKS) {
    ...
    const full = path.join(PUBLIC_ROOT, l.img);
    assert.ok(fs.existsSync(full) && fs.statSync(full).isFile(), `look "${l.id}": image not found at public/${l.img}`);
  }
});
...
test('KA_LOOKS has at least 8 looks (look-1..look-8 per CONTRACT-NEXT.md)', () => {
