/* KHARIS & ALETHEIA — print engine tests
   Run: node --test tests/prints.test.js */

const test = require('node:test');
const assert = require('node:assert');
const KA_PRINTS = require('../prints.js');
...
function checkBalancedTags(svg) {
...
function hasNoNaNOrUndefined(str) {
...
test('sanitize() never throws and always returns the {family,name,palette,params} shape', () => {
  ...
  for (const input of hostileInputs) {
    let result;
    assert.doesNotThrow(() => { result = KA_PRINTS.sanitize(input); }, 'sanitize threw on ' + JSON.stringify(input));
...
test('saved.list()/save()/remove() never throw in node (no localStorage) and stay no-ops', () => {
  ...
  let entry;
  assert.doesNotThrow(() => { entry = KA_PRINTS.saved.save(KA_PRINTS.defaults('ankara')); });
  ...
  let removed;
  assert.doesNotThrow(() => { removed = KA_PRINTS.saved.remove('c_whatever'); });
